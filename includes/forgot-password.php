<?php
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($stmt->num_rows > 0) {
            ...
            $resetLink = "http://localhost:8080/FitZone-Fitness-Center/includes/reset-password.php?token=$token";
            mail($email, "Reset Your Password - FitZone Fitness Center", "Click here to reset your password: $resetLink");

            echo "<script>alert('Reset link has been sent to your email.'); window.location.href='../pages/login.html';</script>";
        } else {
    ...
    <body id="group-classes-body">
        ...
        <div>
            <form method="POST" class="forgot-form">
                <div class="heading">
                    <h2>Forgot Password? <div class="section-underline"><span></span></h2>
                </div>
